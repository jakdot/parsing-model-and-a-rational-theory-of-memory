...
def recall_action(actions, used_spreading, used_retrieval, recently_retrieved, built_constituents, word_freq, label_freq, strength_of_association={}, number_retrieved=3, postulate_gaps=True, reduce_unary=True, decay=0.5, blind=None, prints=True):
    """
    ...
    elif number_retrieved == 1:
        max_activations_ind = np.argpartition(final_array, range(-2, -1))[-1:]
        max_activations = np.sort(final_array[max_activations_ind])
    else:
        raise AttributeError("number_retrieved must be 1 or greater than 1")
    ...
    for i in reversed(range(len(max_activations_ind))):
        ...
        #if activation is positive keep the element
        if max_activation > 0:
            ...
            if prints:
                ...
                for each in spreading:
                    ...
                    if each[0] in pos_dict:
                        string_each1 = lemmatize(string_each1, pos_dict[each[0]])
                    if str(elem[each[0]]) == string_each1:
            ...
            #temp_parser_retrievals.setdefault(str(elem.ACTION), {})
            #action_label_activation = temp_parser_retrievals[str(elem.ACTION)].get(str(elem.ACTION_RESULT_LABEL), 0) + max_activation
            #temp_parser_retrievals[str(elem.ACTION)][str(elem.ACTION_RESULT_LABEL)] = action_label_activation
